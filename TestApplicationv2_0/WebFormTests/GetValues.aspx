<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GetValues.aspx.cs" Inherits="TestApplicationv2_0.WebFormTests.GetValues" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title></title>
</head>
<body>
    <form id="form1" runat="server">
    <div>    
        <result1><asp:Literal ID="Result1Literal" runat="server"></asp:Literal></result1>
        <br />
        <result2><asp:Literal ID="Result2Literal" runat="server"></asp:Literal></result2>
        <br />
        <result3><asp:Literal ID="Result3Literal" runat="server"></asp:Literal></result3>
        <br />
        <result4><asp:Literal ID="Result4Literal" runat="server"></asp:Literal></result4>
    </div>
    </form>
</body>
</html>
